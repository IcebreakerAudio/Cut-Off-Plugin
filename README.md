...
A modern, high-performance VST3 and Standalone audio plugin designed for precise frequency shaping and dynamic control. Cut-Off combines a custom DSP engine with a sleek, real-time reactive neon spectrum visualizer.
...
* **Dynamics Module:** A built-in compressor to tame transients or glue the filtered signal back together. Features adjustable **Attack**, **Decay** (Release), and **Threshold**.
...
* **Build System:** Modern CMake. Bypasses legacy `JuceHeader.h` and Projucer. VST2 backward compatibility is strictly disabled (`JUCE_VST3_CAN_REPLACE_VST2=0`) to ensure clean MSBuild compilation.
...
* **Real-Time Spectrum Visualizer:**
    * **Audio Thread:** Feeds processed Left-channel audio samples into a lock-free FIFO buffer to guarantee zero audio dropouts.
    * **UI Thread:** Uses a 60Hz `juce::Timer` to read the FIFO. Applies a Hann window (`juce::dsp::WindowingFunction`) and calculates frequency bands using Fast Fourier Transform (`juce::dsp::FFT`).
    * **Rendering:** Converts linear FFT data to a logarithmic scale (mimicking human hearing). Drawn dynamically using `juce::Path`, isolated via strict C++ block-scoped clipping regions to prevent graphics state corruption, and filled with custom translucent gradients.
* **Vector UI:**
